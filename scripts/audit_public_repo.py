...
TRACKED_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".bmp", ".gif", ".svg", ".ico", ".wav", ".ogg", ".mp3", ".mp4", ".json", ".xml", ".yaml", ".yml", ".toml", ".csv", ".md", ".txt", ".kt", ".kts", ".java", ".properties"}
PATTERNS = {
    "private_key": re.compile(rb"-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----"),
    "github_token": re.compile(rb"\bgh[pousr]_[A-Za-z0-9_]{20,}\b"),
    "secret_assignment": re.compile(rb"(?i)\b(api[_-]?key|secret|token|password)\s*[:=]\s*['\"][^'\"\r\n]{8,}"),
...
def main() -> int:
    ...
    for root, dirs, names in os.walk(ROOT):
        ...
        for name in names:
            ...
            if path.stat().st_size <= 2_000_000 and path.suffix.lower() not in {".png", ".jpg", ".jpeg", ".webp", ".mp4"}:
                data = path.read_bytes()
                for category, pattern in PATTERNS.items():
                    if pattern.search(data):
                        severity = "high_review_required" if category != "possible_email_data" else "privacy_review_required"
                        findings.append({"path": rel, "category": category, "severity": severity})
    duplicates = [{"sha256": value, "paths": paths} for value, paths in hashes.items() if len(paths) > 1]
    payload = {
    ...
    print(f"Audited {len(files)} files; {len(duplicates)} exact duplicate groups; {len(findings)} review findings")
    return 0
